        ...
        : beta_(beta), gamma_(gamma)
{
    ...
    four_wavelen = wc;          // modal frequency

    imag_frequency = false;     // mother(w) real in  freq-domain
    doublesided    = false;     // analytic: only positive w
...
void GeneralizedMorse::generate(int size)
{
    ...
    if (!mother) {
        std::cerr << "GeneralizedMorse::generate:  failed\n";
        return;
    }
    ...
    for (int k = 0; k < size; ++k) {
        ...
        if (k <= half) {
            ...
            w = (k - size) * dw;             // negative
        }

        if (w <= 0.0f) {
            mother[k] = 0.0f;                // analytic
            continue;
        }
